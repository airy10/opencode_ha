...
def _format_structured_output(
    ...
    """Format the schema to be compatible with OpenCode API."""
    result: JSONSchema = {
        "name": name,
        "strict": True,
    }
...
async def _transform_response(
    ...
    """Transform the OpenCode message to a ChatLog format."""
        ...
        for file_path, mime_type in files:
            ...
            if not mime_type or not mime_type.startswith(("image/", "application/pdf")):
                raise HomeAssistantError(
                    "Only images and PDF are supported by the OpenCode API, "
